...
This dataset is comprised of the intake and outcome record from the Long Beach Animal Shelter during 2017-2024. This analysis focuses on *cats only*. The total number of cats processed between 2017 and 2024 was 'r inline_text(total_longbeach_cats, variable = "n")', only 'r inline_text(nrow(longbeach_cats))' are included in this analysis.See **@tbl-descriptive** for characteristics of the included cats.
...
Overall the participants got a median of `r inline_text(tbl_sex_sleep, variable = "sleep_wknd")` hours of sleep on the weekends. 
There were `r inline_text (tbl_sex_sleep, variable = "region_cat", level = "South", column = "stat_1")` **male** participants from the South region.

Overall between 2017-2024 among the included cats the highest OR for death at outcome was an intake condition of severe at **'r inline_text (tbl_outcomes, variable = "intake", level = "severe", column = "stat_1").
